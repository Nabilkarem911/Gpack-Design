    ...
    <main class="client-portal client-redesign">
      ...
      <section class="client-unified-files-card">
        <div class="unified-files-head">
          ...
          <span class="files-count-badge" id="client-files-count-badge">${totalFiles} ملف</span>
        </div>
        <div class="client-card-scroll client-files-scroll" id="client-files-scroll-list">
          ${renderClientFilesList(d.files)}
        </div>
      </section>
    </main>
  `;
...
function renderClientFilesList(files=[]){
  if(!files.length){
    return '<div class="compact-empty">لا توجد ملفات في المشروع بعد</div>';
  }
  return files.map(renderClientFileRow).join('');
}
